...
>
>- 挂起：协程可以在<span style='color:red'> [ 某个点暂停执行 ]</span>
>- 恢复：协程可以从 <span style='color:red'>[ 挂起的地方继续执行 ]</span>
>
>```csharp
>async def M1():
...
>
>   <span style='color:RED'>[ 未有任务 ]</span> 没有就绪任务时，阻塞在`I/O`多路复用上 <span style='color:red'>[ 等待`I/O`事件触发新任务入队 ]</span>
>
>
><br/>
>
...
>
>- 暂停执行：当遇到`await`时，当前协程暂停，注册到`I/O`等待集合 <span style='color:red'>( 将控制权交还给事件循环 )</span>
>- <span style='color:red'>恢复执行：`I/O`操作完成后，事件循环将该协程移入就绪队列 [ 从挂起点继续运行 ]</span>
>
> 
>
>---
