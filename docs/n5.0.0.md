...
>
>- 任务切换：通过`await`表达式 <span style='color:red'>[ 实现协程间的控制权转移 ]</span>
>
>- 暂停执行：当遇到`await`时，当前协程暂停并让出执行权  <span style='color:red'>[ 将控制权交还给 event loop ( 事件循环 ) ]</span>
>- 恢复执行：被等待的异步操作完成后 <span style='color:red'>[ 调度器会安排该协程继续执行，从挂起点继续运行 ]</span>
>
>
>
>---
